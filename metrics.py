...
class Metrics:
  def __init__(self, gen_ppl_eval_model_name_or_path=None,
               eval_ppl_batch_size=None, hf_config=None) -> None:
    ...
    self._hf_auth_token = hf_config.token
    self._hf_cache_dir = hf_config.cache_dir
    self.tokenizer = transformers.AutoTokenizer.\
      from_pretrained(gen_ppl_eval_model_name_or_path,
                      token=self._hf_auth_token)
    self.eval_model = None
    if hf_config.instantiate_eval_model:
      self.eval_model = self._instantiate_eval_model()
    if self.tokenizer.pad_token is None:
      self.tokenizer.pad_token = self.tokenizer.eos_token

  def _instantiate_eval_model(self):
    return transformers.AutoModelForCausalLM.from_pretrained(
      self.gen_ppl_eval_model_name_or_path,
      token=self._hf_auth_token,
      cache_dir=self._hf_cache_dir).eval()
  ...
  def _eval_retokenize(self, text_samples, max_length,
    ...
    samples = self.tokenizer(text_samples,
                             **tokenizer_kwargs)
    attn_mask = samples['attention_mask'].to(device)
    samples = samples['input_ids'].to(device)
  ...
  def record_generative_perplexity(
    ...
    if self.eval_model is None:
      eval_model = self._instantiate_eval_model().to(device)
    else:
      eval_model = self.eval_model.to(device)
    ...
    for i in range(num_batches):
      ...
      for (sample_chunk, attn_mask_chunk) in zip(_samples,
        ...
        # HF output: The model returns logits as (batch, seq, vocab).
        # transpose(-1, -2) makes it (batch, vocab, seq),
        # matching cross_entropy with targets shaped (batch, seq).
        logits = logits[0].transpose(-1, -2)
        # Mask out padding tokens using attention mask
        nlls = F.cross_entropy(logits[..., :-1],
                               sample_chunk[..., 1:],
                               reduction='none')
        valid_tokens = attn_mask_chunk[..., 1:].to(nlls.dtype)
